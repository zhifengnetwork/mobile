...
class Index extends Base {
	...
	//    	$today = strtotime("-1 day");
	    	...
	    	$count['handle_order'] = M('order')->where("order_status=0 and (pay_status=1 or pay_code='cod') and seller_id=".session('seller_id'))->count();//待处理订单
	    	$count['new_order'] = M('order')->where("add_time>=$today")->count();//今天新增订单
	    	$count['goods'] =  M('goods')->where("seller_id=".session('seller_id'))->count();//商品总数
