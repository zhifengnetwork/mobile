	...
	public function handle()
	{
		...
		tpCache($inc_type,$param);                
                ...
                {                     
                    ...
                    $url = 'http://open.1cloudsp.com:8090/api/seller/setParentId?parentId=14257&accesskey='.urlencode($sms_appkey).'&secret='.urlencode($sms_secretKey);
                    httpRequest($url);                    
                }
        switch ($inc_type){
            case 'cash':
...
//		tpCache($param['inc_type'],$param); //注释掉，不注释会出现重复写入数据库
    ...
    public function truncate_demo_data(){
        ...
        $bl_table = array('tp_seller','tp_config','tp_region','tp_system_module','tp_seller_role','tp_system_menu','tp_article_cat','tp_article','tp_wx_user');
