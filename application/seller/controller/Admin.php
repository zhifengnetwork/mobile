...
class Admin extends Base {
    ...
    public function adminHandle(){
    	...
    	if($r){
			$this->ajaxReturn(['status'=>1,'msg'=>'操作成功','url'=>U('Seller/Admin/index')]);
    ...
    public function roleSave(){
		...
		if($r){
			adminLog('管理角色');
			$this->success("操作成功!",U('Seller/Admin/role_info',array('role_id'=>$data['role_id'])));
		}else{
			$this->error("操作失败!",U('Seller/Admin/role'));
		}
    }
	...
	public function supplierHandle()
	{
		...
		if ($data['act'] == 'add') {
			...
			if ($count) {
				$this->error("此供应商名称已被注册，请更换", U('Seller/Admin/supplier_info'));
			} else {
		...
		if ($r !== false) {
			$this->success("操作成功", U('Seller/Admin/supplier'));
		} else {
			$this->error("操作失败", U('Seller/Admin/supplier'));
		}
	}
}