    ...
    */
    ...
    public function detail()
    {
        ...
        $info = Db::table('tp_team_activity')
            ...
            ->alias('t')
            ->join('goods g','g.goods_id = t.goods_id','left')
            ->field('t.team_id, t.act_name, t.goods_id, t.needer, t.goods_name, t.deleted, t.group_price, t.cluster_type, t.start_time, t.end_time, t.buy_limit, t.sales_sum, t.max_open_num, g.original_img, g.shop_price, g.market_price')
            ->find();
        // dump($info);exit;
        if($info){
            # 对拼团活动状态进行判断
			...
			# 当商品为特定规格商品时，获取价格。
			$spec_price = Db::table('tp_spec_goods_price')->field('price')->find($info['goods_id']);
			$info['shop_price'] = $spec_price ? $spec_price['price'] : $info['shop_price'];
