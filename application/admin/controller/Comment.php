...
class Comment extends Base {
    ...
    public function detail(){
        ...
        if(IS_POST){
            $admin_name = Db::name('admin')->where(['admin_id'=>$this->admin_id])->getField('user_name');
            $add['parent_id'] = $id;
    ...
    public function ajaxindex(){
        ...
        $comment_list = $model->where($where)->order('sort asc,add_time DESC')->limit($Page->firstRow.','.$Page->listRows)->select();
