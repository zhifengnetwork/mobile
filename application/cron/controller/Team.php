...
class Team extends Controller{
    ...
    public function run()
    {
        ...
        $Tf = M('team_found');
        $list = $Tf->field('f.found_id')->alias('f')->join('tp_team_activity t','f.team_id=t.team_id','left')->where('f.found_time between ' . (time()-(3600*48)) . " and (unix_timestamp(now())-t.time_limit*3600) and f.need>0")->select();
        $Tf->alias('f')->join('tp_team_activity t','f.team_id=t.team_id','left')->where('f.found_time between ' . (time()-(3600*48)) . " and (unix_timestamp(now())-t.time_limit*3600) and f.need>0")->update(['f.status'=>3]);
        ...
        foreach($alist as $v2){
            ...
            $apinfo = $AuctionPprice->field('user_id,order_sn')->where(['is_out'=>2,'auction_id'=>$v2['id'],'pay_status'=>['neq',1]])->find(); 
