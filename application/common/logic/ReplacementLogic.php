 ...
 * 活动逻辑类
 */
class ReplacementLogic extends Model
{
	private $userId;//用户id
	private $goodId;//商品id
	...
	**/
	public function distribution()
	{
		...
		$data = array(
			...
			'user_id'  => $distributor['first_leader'],
			'states'   => 102,
		);
		//已有记录,不分
		$log = M('order_divide')->where($data)->find();
        if($log){
            return false;
        }
        $goods = $this->goods();
	...
	{
		//取已返利部分的时间或者取支付时间
		$log_time = M('order_divide')->where('order_id',$this->orderId)->value('add_time');
		if(!$log_time){
			$log_time = M('order')->where('order_id',$this->orderId)->value('pay_time');
			$change_time = $log_time;
			$add_time = date('Y-m-d H:i:s',$log_time);
		}else{
			$change_time = strtotime($log_time);
			$add_time = $log_time;
		}
		$data = array(
			'user_id'=>$userId,
	...
	**/
	...
	public function bonus($meetUser,$price)
	{
		...
		foreach($meetUser as $k => $user){
			...
			$log = M('order_divide')->where($condition)->find();
			if(($log != NULL) && ($is_top == true)){
				break;
			}
			if($log){
				continue;
